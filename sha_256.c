...
#define SHA256_GOROUND(n) 			do { \
										t = SHA256_CALCTEMP(n); \
										values[(123-n)%8] += t; \
										values[(127-n)%8] = t + SIGMA0(values[(120-n)%8]) + MA(values[(120-n)%8],values[(121-n)%8],values[(122-n)%8]); \
									}while(0)
...
static const U32 k_vals[NUM_K_VALUES] = { 0x428a2f98UL, 0x71374491UL, 0xb5c0fbcfUL, 0xe9b5dba5UL, 0x3956c25bUL, 0x59f111f1UL, 0x923f82a4UL, 0xab1c5ed5UL,
...
static const U32 sha256_hvals[NUM_H_VALUES] = { 0x6a09e667UL, 0xbb67ae85UL, 0x3c6ef372UL, 
